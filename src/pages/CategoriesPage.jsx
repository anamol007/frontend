...
export default function CategoriesPage() {
  ...
  // --- Fetch current user (for role gating)
  async function verifyMe() {
    try {
      ...
      // If session invalid, most routes should be protected by router; keep read-only UI
      setMe(null);
    }
  }
  ...
  useEffect(() => {
    verifyMe();         // figure out role
    refresh();          // load categories
    loadCounts();       // load stats
  }, []);
  ...
  async function handleSubmit(form) {
    ...
    if (!isSuper) {
      setErr('Only superadmin can create or edit categories.');
      return;
    }
    try {
      setErr(''); setOk('');
  ...
  async function handleDelete(row) {
    // role gate (defense in depth)
    if (!isSuper) {
      setErr('Only superadmin can delete categories.');
      return;
    }
  ...
  return (
    <div className="space-y-5 bg-gradient-to-br from-slate-50 via-indigo-50 to-emerald-50/40">
      ...
      <div className="rounded-3xl border border-white/60 bg-white/70 p-5 shadow-[0_1px_0_rgba(255,255,255,.6),0_10px_30px_-12px_rgba(2,6,23,.25)] backdrop-blur-xl">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">
                Organize products by category and see product counts.
              </p>
            </div>
          </div>
          ...
          <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto">
            ...
            {/* read-only users still can refresh data */}
            {!isSuper && (
              <button
                onClick={()=>{ refresh(); loadCounts(); }}
                className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-700 shadow-sm transition hover:bg-slate-50"
      ...
      {/* Modal for create/edit (superadmin only) */}
      {isSuper && (
        <FormModal
