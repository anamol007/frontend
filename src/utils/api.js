...
const LOGIN_PATH = (process.env.REACT_APP_LOGIN_PATH || '/users/login'); // per doc: POST /users/login
...
// login: POST /users/login -> { success, message, data: { token, user } }
...
export const fetchLowStock = (kgThreshold = 10, boriThreshold = 5) =>
  api.get('/stock/low', { params: { kgThreshold, boriThreshold } }).then(r => r?.data?.data ?? r?.data ?? []);